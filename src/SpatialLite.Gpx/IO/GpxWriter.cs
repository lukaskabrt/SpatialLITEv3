...
public class GpxWriter : IDisposable, IGpxWriter
{
    ...
    public void Write(GpxRoute route)
    {
        ...
        if (Settings.WriteMetadata && route.Metadata != null)
        {
    ...
    public void Write(GpxTrack track)
    {
        ...
        for (var i = 0; i < track.Geometries.Count; i++)
        {
            _xmlWriter.WriteStartElement("trkseg");

            for (var ii = 0; ii < track.Geometries[i].Points.Count; ii++)
            {
        ...
        if (Settings.WriteMetadata && track.Metadata != null)
        {
    ...
    private void WritePoint(GpxPoint point, string pointElementName)
    {
        ...
        {
            _xmlWriter.WriteElementString("ele", point.Elevation.Value.ToString(_invariantCulture));
        }
        ...
        if (Settings.WriteMetadata && point.Metadata != null)
        {
